            ...
            <div className="rounded-2xl shadow-md bg-white border border-neutral-200">
              ...
              <div className="px-6 pb-6 grid sm:grid-cols-2 gap-3.5 text-sm">
                <Feature
                  icon={<ServerIcon />}
                  title="Servers & POS"
                  desc="All of your critical devices — healthy, patched, backed up."
                />
                <Feature
                  icon={<ShieldIcon />}
                  title="Security"
                  desc="EDR/MDR, least-privilege, and compliance-minded baselines."
                />
                <Feature
                  icon={<NetworkIcon />}
                  title="Networks"
                  desc="Switches, APs, firewalls tuned for lobby & auditorium loads."
                />
                <Feature
                  icon={<CableIcon />}
                  title="Hardware Lifecycle"
                  desc="Warranty tracking and refresh planning before EOL."
                />
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <header className="mb-8">
            ...
            <p className="mt-2 text-neutral-600">
              Built for the unique cadence of cinema operations.
            </p>
          </header>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            <ServiceCard
              ...
              bullets={["24/7 alerts", "Patch orchestration", "Asset inventory & reporting"]}
            />
            <ServiceCard
              icon={<ShieldIcon />}
              title="Endpoint Detection & Response"
              bullets={["EDR/MDR rollout", "Threat hunting", "Policy hardening"]}
            />
            <ServiceCard
              icon={<NetworkIcon />}
              title="Network Cleanup & Design"
              bullets={["Rack remediation", "Wi-Fi tuning", "Firewall best practices"]}
            />
            <ServiceCard
              icon={<CableIcon />}
              title="Lifecycle & Warranty"
              bullets={["EOL planning", "Warranty extensions", "Cost-avoidance strategy"]}
            />
            <ServiceCard
              icon={<MailIcon />}
              title="Backup & Recovery"
              bullets={["Server backups", "Config snapshots", "Restore runbooks"]}
            />
            <ServiceCard
              icon={<PhoneIcon />}
              title="On-call & Projects"
              bullets={[
                "Go-live coverage",
                "POS migrations",
                "On-prem server migrations",
                "Vendor management",
              ]}
            />
          </div>
          ...
          <div className="lg:col-span-2">
            ...
            <ul className="mt-6 space-y-2 text-neutral-700">
              <li className="flex gap-2">
                <CheckIcon className="w-5 h-5 mt-0.5" /> Project management that respects opening weekends
              </li>
              <li className="flex gap-2">
                <CheckIcon className="w-5 h-5 mt-0.5" /> Vendor wrangling so you don’t have to
              </li>
              <li className="flex gap-2">
                <CheckIcon className="w-5 h-5 mt-0.5" /> Clear reporting, no black boxes
              </li>
            </ul>
          </div>
...
function SmokeTests() {
  useEffect(() => {
    const tests: Array<[string, boolean]> = [
      ...
      ["Testimonials section exists", !!document.getElementById("testimonials")],
